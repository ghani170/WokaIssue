...
class DeveloperController extends Controller
{
    ...
    {
        // Pastikan ini developer
        if ($developer->role !== 'developer') {
            return redirect()->route('admin.developer.index')
                ->with('error', 'User ini bukan developer.');
        }

        // Cek apakah developer masih punya laporan
        if ($developer->laporan()->count() > 0) {
            return redirect()->route('admin.developer.index')
                ->with('error', 'Developer tidak bisa dihapus karena masih menangani laporan.');
        }
